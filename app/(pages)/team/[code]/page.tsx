...
export default function Team({ params }: { params: { code: string } }) {
  ...
  const handleVote = async () => {
    ...
    if (res.ok) alert("Vote submitted successfully!");
      ...
      alert(
        "Failed to vote. You may have already voted or the input was invalid.",
      );
  };
                ...
                {scores
                  .sort(
                    (a, b) =>
                      b.audience_score +
                      b.bonus_score -
                      (a.audience_score + a.bonus_score),
                  )
                  .map((score, index) => (
                    <motion.div
                      key={score.id}
                    ...
                    >
                      ...
                      <div className="text-emerald-400 font-black text-3xl">
                        {score.audience_score + score.bonus_score}{" "}
                        <span className="text-base font-medium text-gray-500 uppercase tracking-widest">
                          pts
                        </span>
                      </div>
                    </motion.div>
