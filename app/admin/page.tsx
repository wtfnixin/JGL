...
export default function Admin() {
  ...
  useEffect(() => {
    ...
    const pingInterval = setInterval(() => { fetch('/api/ping').catch(() => {}) }, 2 * 60 * 1000);
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-black flex items-center justify-center p-6 text-[#ededed] font-mono selection:bg-[#00FF41] selection:text-black">
        <form onSubmit={handleLogin} className="border-2 border-[#333] p-8 w-full max-w-md relative bg-black">
          ...
          <div className="mb-8 border-b-2 border-[#333] pb-4">
             <h1 className="text-2xl font-bold tracking-tighter uppercase mb-1">System Override</h1>
             <p className="text-[#666] text-xs tracking-widest uppercase">Target: Event Control Deck</p>
          </div>

          <div className="space-y-6">
            <div>
              <label className="block text-[#00FF41] text-xs font-bold mb-2 tracking-[0.2em] uppercase">Auth Key</label>
              <input 
                type="password" 
                value={pass}
    ...
    <div className="min-h-screen bg-black p-6 md:p-8 lg:p-12 text-[#ededed] font-mono selection:bg-[#00FF41] selection:text-black">
      <div className="max-w-[1400px] mx-auto">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center border-b-2 border-[#333] pb-6 mb-8">
           <div>
             <h1 className="text-3xl font-bold uppercase tracking-tighter text-[#00FF41]">Admin.Control</h1>
             <p className="text-[#666] text-xs tracking-widest mt-1 uppercase">Jain's Got Latent — Live Event Switchboard</p>
           </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-4 flex flex-col gap-8">
            ...
            <div className="border-2 border-[#333] p-6 relative">
              <h2 className="text-[#00FF41] text-xs font-bold mb-6 tracking-[0.2em] uppercase flex items-center gap-2">
                <span className={`w-2 h-2 block ${runningTask ? 'bg-yellow-500' : 'bg-[#00FF41] animate-pulse'}`}></span>
                Phase Controls
              </h2>
            ...
            {/* Add Teams Box */}
            <div className="border-2 border-[#333] p-6 relative">
              <h2 className="text-[#00FF41] text-xs font-bold mb-6 tracking-[0.2em] uppercase">Add Teams</h2>
          ...
          <div className="lg:col-span-4 h-full border-2 border-[#333] p-6 flex flex-col max-h-[80vh]">
            <h2 className="text-[#00FF41] text-xs font-bold mb-6 tracking-[0.2em] uppercase">Roster / Audience Push</h2>
            
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              {teams.length === 0 && (
                <div className="text-[#444] text-xs border border-[#333] p-4 text-center">NO TEAMS ESTABLISHED</div>
              )}
              {teams.map((team) => {
                const isOnStage = gameState.current_team_id === team.id;
                
                return (
                  <div key={team.id} className={`border p-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 transition-all ${isOnStage ? 'border-[#00FF41] bg-[#00FF41]/10' : 'border-[#333] hover:border-[#666]'}`}>
                     <div className="flex flex-col">
                        <span className={`font-bold uppercase tracking-widest ${isOnStage ? 'text-[#00FF41]' : 'text-[#ededed]'}`}>{team.name}</span>
          ...
          <div className="lg:col-span-4 h-full border-2 border-[#333] p-6 flex flex-col justify-center items-center text-center">
            <h2 className="text-[#00FF41] text-xs font-bold mb-6 tracking-[0.2em] uppercase">Big Screen Access</h2>
            <p className="text-[#666] text-xs uppercase tracking-widest mb-12">
              Launch the dedicated, full-screen leaderboard interface on a projector or external display.
            </p>
            <a 
              href="/results" 
              target="_blank" 
...
function PhaseButton({ label, taskId, runningTask, onClick, active = false, alert = false }: any) {
  const isRunning = runningTask === taskId;
  let baseColor = "border-[#333] text-[#ededed] hover:bg-[#222]";
  
  if (active) baseColor = "border-[#00FF41] text-[#00FF41] bg-[#00FF41]/10 hover:bg-[#00FF41] hover:text-black";
  if (alert && active) baseColor = "border-red-500 text-red-500 bg-red-500/10 hover:bg-red-500 hover:text-white";
  else if (alert) baseColor = "border-[#333] text-red-500 hover:bg-red-500 hover:text-white";

  if (isRunning) baseColor = "border-yellow-500 text-yellow-500 bg-yellow-500/10 cursor-wait";
