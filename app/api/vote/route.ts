...
export async function POST(req: Request) {
  try {
    ...
    if (!performing_team_id || typeof performing_team_id !== 'string' ||
        !voting_team_id || typeof voting_team_id !== 'string' ||
        typeof points_given !== 'number') {
      return Response.json({ error: 'Missing or invalid fields' }, { status: 400 })
    }

    if (!Number.isInteger(points_given) || points_given < 1 || points_given > 10) {
      return Response.json({ error: 'Points must be an integer between 1 and 10' }, { status: 400 })
    }

    if (voting_team_id === performing_team_id) {
      return Response.json({ error: 'Cannot vote for your own team' }, { status: 400 })
    }
    ...
    if (performing_team_id !== gameState.current_team_id) {
      return Response.json({ error: 'This team is not currently on stage' }, { status: 400 })
    }
